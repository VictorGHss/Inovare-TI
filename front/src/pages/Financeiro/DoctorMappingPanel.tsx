      ...
      <header className="border-b border-slate-100 bg-gradient-to-r from-brand-secondary/30 to-transparent px-6 py-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-800">Mapeamento de Médicos</h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {syncingBase
                ...
                : <RefreshCcw size={15} />}
              {syncingBase ? 'Sincronizando...' : 'Sincronizar Conta Azul'}
            </button>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              {['Nome do Médico', 'UUID Conta Azul', 'E-mail de Destino', 'Ações'].map((col, i) => (
                <th
                  key={col}
                  className={`px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 ${i === 3 ? 'text-center' : 'text-left'}`}
                >
                  {col}
                </th>
              ))}
              ...
              mapeamentosOrdenados.map((mapeamento) => (
                ...
                >
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <button
                      type="button"
                      onClick={() => { void handleRemover(mapeamento.id); }}
                      disabled={idRemovendo === mapeamento.id}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50/60 px-3 py-1.5 text-xs font-semibold text-red-600 transition-colors hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-70"
                    >
